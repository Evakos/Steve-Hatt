  ...
  return (
    <>
      ...
      <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((p) => {
          const eligible = isEligibleForMode(p, mode);
          return (
            <Link
              key={p.slug}
              href={`/shop/${p.slug}`}
              className={`group border border-border bg-white p-4 transition-all hover:border-navy/30 hover:shadow-md ${!eligible ? "opacity-60" : ""}`}
            ...
            >
              ...
              {!eligible && (
                <p className="mt-2 text-xs text-text-light">Not available for Christmas pre-order</p>
              )}
            </Link>
          );
        })}
